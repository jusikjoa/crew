...
- **동작 절차**:
  ...
  2. 요청 본문에서 채널 정보(이름, 설명, 공개 여부, 비밀번호)를 받습니다.
  3. 채널명 중복 여부를 확인합니다.
  4. 비밀번호가 제공된 경우 bcrypt로 해시화합니다.
  5. 새 채널을 생성하고 `createdBy`에 사용자 ID를 저장합니다.
  6. 생성자를 채널의 `members`에 자동으로 추가합니다.
  7. 생성된 채널 정보를 반환합니다 (비밀번호는 제외).
- **요청 본문 예시**:
  ```json
  {
...
- **동작 절차**:
  ...
  8. 비밀번호가 제공된 경우 bcrypt로 해시화합니다.
  9. 채널 정보를 업데이트하고 반환합니다 (비밀번호는 제외).
...
- **동작 절차**:
  ...
  2. URL 파라미터에서 채널 ID를 받습니다.
  3. 채널을 조회합니다.
  4. **비공개 채널(`isPublic === false`)인 경우**:
     - 채널에 비밀번호가 설정되어 있으면, 요청 본문의 비밀번호와 비교합니다.
     - 비밀번호가 없으면 401 Unauthorized 에러를 반환합니다.
     - 비밀번호가 틀리면 401 Unauthorized 에러를 반환합니다.
     - 비밀번호가 설정되지 않은 비공개 채널은 403 Forbidden 에러를 반환합니다.
  5. 채널과 사용자를 조회합니다.
  6. 이미 멤버인지 확인합니다 (이미 멤버인 경우 409 Conflict 에러).
  7. 채널의 `members`에 사용자를 추가합니다.
  8. 사용자의 `channels`에 채널을 추가합니다.
  9. 업데이트된 채널 정보를 반환합니다.
- **요청 본문 예시 (비공개 채널 참여 시)**:
  ```json
  {
    "password": "channel123"
  }
...
- **요청 본문 예시 (공개 채널 참여 시)**:
  ...
  ```
- **에러 응답**:
  - `401 Unauthorized`: 비공개 채널에 비밀번호가 없거나 틀린 경우
  - `403 Forbidden`: 비밀번호가 설정되지 않은 비공개 채널에 참여 시도한 경우
  - `409 Conflict`: 이미 참여 중인 채널인 경우
...
- **채널 참여**: 
  - 공개 채널(`isPublic === true`): 비밀번호 없이 참여 가능
  - 비공개 채널(`isPublic === false`): 비밀번호가 설정된 경우 올바른 비밀번호 필요
  - 비밀번호가 설정되지 않은 비공개 채널: 참여 불가
...
# JWT 시크릿 키 (프로덕션 환경에서는 반드시 변경하세요)
JWT_SECRET=your-secret-key-change-in-production
```

### Postman 컬렉션에서 포트 설정

Postman 컬렉션을 사용하는 경우, 컬렉션 변수에서 `port` 값을 환경에 맞게 변경할 수 있습니다:
1. Postman에서 컬렉션을 엽니다.
2. Variables 탭에서 `port` 변수 값을 수정합니다 (기본값: 3000).
...
서버는 `.env` 파일의 `PORT` 환경 변수 값으로 실행됩니다. 환경 변수가 설정되지 않은 경우 기본값인 3000번 포트로 실행됩니다.
